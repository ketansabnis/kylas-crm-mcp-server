#!/usr/bin/env python3
"""Assemble fresh_2026-07-20.json + current_data.json from merged deal_fields
and notes pulls (Run 10, Mon 20-Jul-2026)."""
import json, os, glob, datetime
BASE = os.path.dirname(os.path.abspath(__file__))
RUN = "2026-07-20"
run_dt = datetime.date(2026, 7, 20)

DF = {}
for p in sorted(glob.glob(os.path.join(BASE, "deal_fields_2026-07-20_*.json"))):
    DF.update(json.load(open(p)))
NT = {}
for p in sorted(glob.glob(os.path.join(BASE, "notes_2026-07-20_*.json"))):
    NT.update(json.load(open(p)))
DF = {int(k): v for k, v in DF.items()}
NT = {int(k): v for k, v in NT.items()}
ids = list(DF.keys())

PRIOR = json.load(open(os.path.join(BASE, "snapshot_2026-07-16.json")))
pby = {r["id"]: r for r in PRIOR["deals"]}

# ---- membership sets from live search_entity (RUN_DATE = 2026-07-20) ----
overdue_task = {4499510,4478489,4370308,4241966,4230613,3575524,3302998}
future_task  = {4535970,4535312,4535079,4516021,4515897,4511499,4511483,4510268,4510214,4508480,
                4482302,4482197,4479599,4479594,4458771,4458749,4449587,4446753,4438366,4435090,
                4402389,4402374,4389699,4389674,4375201,4368709,4233760,4233715,4144089,3968505,3317883}
idle5 = {4510192,4494717,4475303,4475289,4230613,3304063,3304062,3302998}
idle7 = {4494717,4475303,4475289,4230613,3304063,3304062,3302998}
idle15 = set()
overdue_golive_set = {4475303,4475289,4458771,4458749,4437383,4389674,4375201,4368709,4337048,4255681,4230613,3575524}
not_signedup_set   = {4545857,4538512,4510267,4475303,4475289,4368709}
...
for i in ids:
    ...
    golive_overdue = i in overdue_golive_set
    not_su = i in not_signedup_set
    recs.append({
...
OUT = {"report_date": RUN,
       "run_label": "Run 10 (Mon 20-Jul-2026) vs 16-Jul & 13-Jul",
       "run_index": 10, "pipeline_id": 27474,
...
json.dump(fresh, open(os.path.join(BASE, "fresh_2026-07-20.json"), "w"), indent=1)
...
print("DROPPED since 16-Jul:", [(i, pby[i]['name']) for i in dropped])
